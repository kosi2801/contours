...
public class LongTypeGenerator extends AbstractTypeGenerator {
    ...
    public void generateDataStructureCode(StringBuilder fileContent, ClassAttribute attr) {
        ...
        fileContent.append(generateBasicTypeAttributeCode(multiplicity, attr.static_multiplier, "Long", attr.name));
    }
    ...
    public void generateAttributeReaderCode(StringBuilder fileContent, ClassAttribute attr) {
        ...
        if (StringUtils.isBlank(attr.name) && StringUtils.isBlank(attr.contextName)) {
            ...
            if (attr.context_multiplier != null) {
                fileContent.append("        ParseUtil.skipBytes(is, context, context.variables.get(\"" + attr.context_multiplier + "\") * 8); \n");
            } else {
                fileContent.append("        ParseUtil.skipBytes(is, context, " + attr.static_multiplier.toString() + " * 8); \n");
            }
        } else {
            // get dynamic number of elements from context
            if (attr.context_multiplier != null) {
                fileContent.append("        final Long[] " + attr.syntheticName + " = new Long[context.variables.get(\"" + attr.context_multiplier
                        + "\")]; \n");
            } else { // get static number of elements
                fileContent.append("        final Long[] " + attr.syntheticName + " = new Long[" + attr.static_multiplier.toString() + "]; \n");
            }
            fileContent.append("        for(int i=0; i<" + attr.syntheticName + ".length;i++){\n");
            fileContent.append("            " + attr.syntheticName + "[i] = ParseUtil.readLong(is, context); \n");
            fileContent.append("        }\n");
            if (attr.name != null) {
            ...
            if (attr.contextName != null) {
                fileContent.append("        context.variables.put(\"" + attr.contextName + "\", " + attr.syntheticName + "[0].intValue());\n");
            }
        }
    }
